...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT name, contact, app_date AS date, app_time AS time FROM Appointment WHERE apID = ?";
    ...
    if (sqlsrv_execute($stmt)) {
        ...
        if ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            echo json_encode(array(
                "patient_name" => $row['name'], // Change key to match JavaScript expectations
                "contact" => $row['contact'],
                "app_date" => $date, // Change key to match JavaScript expectations
                "app_time" => $time  // Change key to match JavaScript expectations
            ));
        } else {
